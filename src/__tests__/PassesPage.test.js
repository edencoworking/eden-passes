...
describe("PassesPage (localStorage integration)", () => {
  ...
  test("selecting existing customer via suggestions does not duplicate customer", () => {
    ...
    const customers = JSON.parse(
      localStorage.getItem("EDEN_CUSTOMERS") || "[]",
    );
  ...
  test("empty state message appears when no passes", () => {
    seedStorage({
      customers: [{ id: "c1", name: "John Doe" }],
      passes: [],
    });
    render(<PassesPage />);
    expect(
      screen.getByText(/No passes yet. Create one above./i),
    ).toBeInTheDocument();
  });
  ...
  test("new pass prepends to list (most recent first)", () => {
    ...
    // Create new pass
    fireEvent.change(screen.getByLabelText(/Pass Type/i), {
      target: { value: "hourly" },
    });
    fireEvent.change(screen.getByLabelText(/Customer Name/i), {
      target: { value: "Zeta User" },
    });
